...
export default function App() {
  ...
  async function checkSelectedHotelLivePrice(hotel) {
    ...
    try {
      ...
      if (data.live_payment_ready && data.first_rate) {
        updated.price = data.first_rate.selling_rate || data.first_rate.net || hotel.price;
        updated.currency = data.first_rate.currency || hotel.currency;
      }

      setSelectedHotel(updated);
      setHotels((current) =>
        current.map((x) =>
          (x.hotel_id || x.id) === (hotel.hotel_id || hotel.id) ? updated : x
        )
      );

      return updated;
    } catch {
      setMessage("We could not check the latest live price for this hotel. You can still send a reservation request.");
      return hotel;
    } finally {
  ...
  async function sendRequest() {
    ...
    try {
      const payload = {
        ...
        amount: selectedHotel.first_rate?.selling_rate || selectedHotel.first_rate?.net || "",
        currency: selectedHotel.first_rate?.currency || "",
        amount: selectedHotel.price || selectedHotel.selling_rate || selectedHotel.amount || "",
        currency: selectedHotel.currency || "",
        price_display: selectedHotel.price
          ? `${selectedHotel.currency || ""} ${selectedHotel.price}`.trim()
          : "Latest price to be confirmed",
      };
        ...
        <div style={styles.twoCol}>
          <div>
            ...
            <div style={styles.results}>
              {hotels.map((hotel) => (
                <div key={hotel.hotel_id || hotel.id} style={selectedHotel?.hotel_id === hotel.hotel_id ? styles.cardSelected : styles.card} onClick={() => {
                    setSelectedHotel(hotel);
                    checkSelectedHotelLivePrice(hotel);
                  }}>
                  <HotelImage hotel={hotel} />
                  <h3 style={styles.hotelName}>{hotel.hotel_name || hotel.name}</h3>
                  <p>{hotel.area || hotel.address || hotel.city}</p>
                  <div style={hotel.live_payment_ready ? styles.priceLine : styles.pricePending}>
                    {hotel.live_payment_ready
                      ? `${hotel.first_rate.currency || hotel.currency || ""} ${hotel.first_rate.selling_rate || hotel.first_rate.net || hotel.price}`.trim()
                      : (hotel.price_status || "Latest price and room availability will be confirmed")}
                  </div>
                  <div style={hotel.live_payment_ready ? styles.rateGood : styles.ratePending}>
                    {hotel.live_payment_ready ? "hotel.price_status || "Live room price available"" : "Reservation request required"}
                  </div>
                </div>
              ))}
            </div>
          ...
          <aside style={styles.reserve}>
            ...
            <div style={styles.selectedBox}>
              {selectedHotel ? (selectedHotel.hotel_name || selectedHotel.name) : "Choose a stay"}
              {selectedHotel?.price && (
            ...
            <button style={styles.goldButton} onClick={sendRequest}>
              {checkingLivePrice ? "Checking latest price..." : selectedHotel?.live_payment_ready ? "Continue securely" : "Send reservation request"}
            </button>
            <div style={styles.safeNote}>
              {checkingLivePrice
                ? "Checking the latest room price for this selected property..."
                : selectedHotel?.live_payment_ready
                  ? "Live price is ready. Continue securely when you are ready."
                  : "Our reservations team will help confirm the latest availability, price, and next steps."}
            </div>
          </aside>
        </div>
      </section>
...
const styles = {
  ...
  priceLine: { background: "#eef4ff", borderRadius: 12, padding: 12, margin: "12px 0", fontWeight: 900, fontSize: 18, color: "#07111f" },
  pricePending: { background: "#fff2be", borderRadius: 12, padding: 12, margin: "12px 0", fontWeight: 900, fontSize: 16, color: "#7a4b00" },
  selectedPrice: { marginTop: 10, color: "#075b24", fontSize: 18 },
