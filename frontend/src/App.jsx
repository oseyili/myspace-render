...
export default function App() {
  ...
  async function handleSearch(event) {
    ...
    try {
      ...
      const requestUrl = `${API_BASE}/api/hotels?${params.toString()}`;
      console.log("Hotel search URL:", requestUrl);
      ...
      setStatusText(
        items.length
          ? payload.message || `Showing ${items.length} live hotel results for ${cityValue}.`
          : payload.detail || `No hotel results matched this search for ${cityValue}.`
      );
    } catch (error) {
      console.error("Hotel search failed:", error);
      setHotels([]);
      setErrorText(error?.message || "Live hotel search could not connect just now.");
          ...
          {!visibleHotels.length ? (
            ...
            >
              {visibleHotels.map((hotel, index) => {
                const hotelKey =
                  hotel.id || hotel.hotel_id || hotel.name || `hotel-${index}`;
