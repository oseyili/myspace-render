...
export default function App() {
  ...
  async function checkLivePriceForHotel(hotel) {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE}/api/hotels/live-check?${params.toString()}`);
      ...
      if (updated.live_payment_ready) {
        setMessage("Live price is ready. Continue securely when ready.");
      } else {
        setMessage(data.message || "Latest price will be confirmed before payment.");
      }
    } catch {
      ...
      <section style={styles.right}>
        <div style={styles.searchBox}>
          ...
          <div style={styles.dateGrid}>
            <label>Check-in<input style={styles.input} type="date" min={today} value={checkin} onChange={(e) => setCheckin(e.target.value)} /></label>
            <label>Check-out<input style={styles.input} type="date" min={checkin || today} value={checkout} onChange={(e) => setCheckout(e.target.value)} /></label>
          </div>
              ...
              <div style={selectedHotel.live_payment_ready ? styles.rateGood : styles.safeNote}>
                {checkingLivePrice
                  ...
                  : selectedHotel.live_payment_ready
                    ? `Live total ready: ${selectedHotel.currency || ""} ${selectedHotel.price || ""}`.trim()
