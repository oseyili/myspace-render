...
const GUIDE_CONTENT = {
  Lagos: {
    ...
    intro: "Lagos is one of Africa’s most exciting cities: fast, creative, coastal, business-focused, and full of music, food, art, beaches, and premium neighbourhoods.",
    experiences: [
      ...
      "Visit Nike Art Gallery, Landmark Beach, and Lekki Conservation Centre.",
      "Try jollof rice, suya, pepper soup, grilled fish, and modern Nigerian dining."
    ],
    practical: [
  ...
  Abuja: {
    ...
    intro: "Abuja is spacious, calmer than Lagos, and excellent for government visits, diplomatic travel, conferences, family stays, and comfortable city breaks.",
    experiences: [
  ...
  London: {
    ...
    intro: "London is built for memorable trips: world-class museums, West End theatre, luxury shopping, royal parks, football, history, food markets, and neighbourhoods with very different personalities.",
    experiences: [
  ...
  Paris: {
    ...
    intro: "Paris offers art, food, design, fashion, luxury hotels, historic streets, and neighbourhoods made for walking.",
    experiences: [
  ...
  Dubai: {
    ...
    intro: "Dubai offers polished hospitality, iconic architecture, beaches, shopping, restaurants, theme parks, desert experiences, and high-service hotels.",
    experiences: [
...
function guideFor(city) {
  return GUIDE_CONTENT[city] || {
    ...
    intro: `${cityLabel(city)} offers different experiences depending on your neighbourhood. Compare hotels by area, comfort, access, images, and travel purpose before choosing your stay.`,
    experiences: [
...
export default function App() {
  ...
  async function searchHotels() {
    ...
    try {
      ...
      setMessage(
        list.length
          ? `${list.length} stays found in ${cityLabel(city)}. Choose a property to refresh live rate.`
          : "No real stays found. Try a nearby city, area, or different keyword."
      );
    } catch {
  ...
  async function checkLivePriceForHotel(hotel) {
    ...
    setMessage("Checking latest live room price for this selected stay...");
    ...
    try {
      ...
      params.set("amount", String(hotel.price || hotel.first_rate?.display_amount || ""));
      ...
      setMessage(updated.live_payment_ready ? "Live price is ready. Currency converter is available." : "Latest price will be confirmed before payment.");
    } catch {
  ...
  async function sendRequest() {
    ...
    try {
      const payload = {
        ...
        rate_key: selectedHotel.first_rate?.rate_key || selectedHotel.hotel_id || selectedHotel.id || "",
      };
  ...
  if (page === "faq") {
    return (
      <InfoPage onBack={() => setPage("home")} title="Frequently asked questions">
        <p><b>How do I reserve?</b><br />Choose a destination, compare stays, refresh the selected live rate, submit your details, and our reservations team confirms the latest availability and next steps.</p>
        <p><b>Why request before payment?</b><br />Some global properties require current supplier confirmation before payment. This protects customers from outdated availability or stale prices.</p>
        <p><b>Can I convert the price?</b><br />Yes. Select a hotel, refresh the live rate, choose a target currency, and use the currency converter.</p>
      </InfoPage>
    );
  }
  ...
  if (page === "terms") {
    return (
      <InfoPage onBack={() => setPage("home")} title="Booking terms">
        <p>Hotel conditions, prices, taxes, cancellation terms, and availability can vary by property, room type, dates, and supplier.</p>
        <p>Displayed currency is shown for customer clarity. Final payment currency and supplier conditions must be confirmed before payment.</p>
        <p>MySpace Hotel avoids fake property images and generic hotel placeholders because customer trust is essential.</p>
      </InfoPage>
    );
  }
  ...
  if (page === "support") {
    return (
      <InfoPage onBack={() => setPage("home")} title="Customer support">
        <p>For reservation help, email <b>reservations@myspace-hotel.com</b>.</p>
        <p>Include your destination, hotel name, travel dates, number of guests, room needs, and contact email.</p>
        <p>For urgent travel support, ask your hotel for the nearest hospital, police assistance, pharmacy, and trusted local transport.</p>
      </InfoPage>
    );
  }
  ...
  return (
    <div style={styles.page}>
      <section style={styles.left}>
        ...
        <p style={styles.sub}>Compare real properties, choose the right neighbourhood, refresh live rates, convert currency, and send a safer reservation request.</p>
      ...
      <section style={styles.right}>
        <div style={styles.searchBox}>
          ...
          <p style={styles.copy}>Choose your country, city, dates, and preferred area. Then compare hotels, refresh live rates, and continue with a reservation request.</p>

          <label style={styles.fieldLabel}>Country</label><select style={styles.input} value={country} onChange={(e) => {
          ...
          <label style={styles.fieldLabel}>City</label><select style={styles.input} value={city} onChange={(e) => setCity(e.target.value)}>
        ...
        <div style={styles.twoCol}>
          <div>
            <div style={styles.label}>AVAILABLE STAYS AND LIVE RATES</div>
            ...
            <div style={styles.results}>
              {hotels.map((hotel) => (
                <div key={hotel.hotel_id || hotel.id} style={selectedHotel?.hotel_id === hotel.hotel_id ? styles.cardSelected : styles.card} onClick={() => checkLivePriceForHotel(hotel)}>
                  ...
                  <div style={styles.priceLine}>{hotel.currency || hotel.first_rate?.display_currency || ""} {hotel.price || hotel.first_rate?.display_amount || ""}</div>
                  <div style={hotel.live_payment_ready ? styles.rateGood : styles.safeNote}>
                    {hotel.price_status || "Click to refresh live rate."}
                  </div>
                </div>
              ))}
            </div>
          ...
          <aside style={styles.reserve}>
            ...
            {selectedHotel && (
              <div style={selectedHotel.live_payment_ready ? styles.rateGood : styles.safeNote}>
                {checkingLivePrice
                  ? "Checking latest live room price..."
                  : selectedHotel.live_payment_ready
                    ? `Live total ready: ${selectedHotel.currency || selectedHotel.first_rate?.display_currency || ""} ${selectedHotel.price || selectedHotel.first_rate?.display_amount || ""}`.trim()
            ...
            <button style={styles.goldButton} onClick={sendRequest}>{checkingLivePrice ? "Checking latest price..." : "Send reservation request"}</button>
...
const styles = {
  fieldLabel: { display: "block", marginTop: 12, marginBottom: 4, fontSize: 16, fontWeight: 900, color: "#07111f" },
