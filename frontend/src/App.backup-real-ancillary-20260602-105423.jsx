      ...
      <div style={styles.cardGrid}>
        {options.map((option) => (
          <div key={option.id} style={styles.infoCard}>
            ...
            <div style={styles.priceLine}>{option.currency} {money(option.price)}</div>
...
function TransfersPortal(props) {
  ...
  return (
    <PortalShell title="Airport Transfers" subtitle="Help customers move between airport and hotel with confidence." badge="Transfers">
      {notice ? <div style={styles.notice}>{notice}</div> : null}
      <div style={styles.cardGrid}>
        {options.map((option) => (
          <div key={option.id} style={styles.infoCard}>
      ...
      <div style={styles.cardGrid}>
        {attractions.map((item) => (
          <div key={item.id} style={styles.infoCard}>
            ...
            <div style={styles.priceLine}>{item.currency} {money(item.price)}</div>
...
function FeaturedHotelsPortal() {
  ...
  return (
    <PortalShell title="Featured Hotel Advertising" subtitle="Hotels can request promoted visibility on MySpace Hotel." badge="Hotel growth">
      <div style={styles.cardGrid}>
        <InfoCard title="Bronze - £49/month" text="Entry-level promoted visibility for selected destinations." />
        <InfoCard title="Silver - £99/month" text="Stronger search and destination visibility for participating hotels." />
        <InfoCard title="Gold - £199/month" text="Priority promotion for higher-value destinations and campaigns." />
        <InfoCard title="Platinum - £499/month" text="Premium promotion for hotels seeking stronger brand visibility." />
      </div>
      ...
      <form style={styles.form} onSubmit={submit}>
        ...
        <select style={styles.input} value={packageName} onChange={(e) => setPackageName(e.target.value)}>
          <option value="BRONZE">Bronze - £49/month</option>
          <option value="SILVER">Silver - £99/month</option>
          <option value="GOLD">Gold - £199/month</option>
