...
function buildDestinationsFromHotels(hotels) {
  const preferred = [
    ...
    { country: "Italy", cities: [{ city: "Rome", currency: "EUR" }] },
    { country: "Türkiye", cities: [{ city: "Istanbul", currency: "TRY" }] },
    { country: "Czech Republic", cities: [{ city: "Prague", currency: "CZK" }] }
  ];
  ...
  function add(country, city, currency) {
    ...
    city = clean(city);
    currency = clean(currency || CITY_CURRENCY[norm(city)] || "GBP").toUpperCase();

    if (!country || !city) return;
    if (country.toLowerCase() === "worldwide") return;
    if (city.toLowerCase() === "global") return;
  ...
  for (let i = 0; i < Math.min(hotels.length, 250000); i++) {
  ...
  countries.sort((a, b) => {
    const order = ["Nigeria", "United Kingdom", "France", "Spain", "United Arab Emirates", "United States"];
...
app.get("/api/hotels/selected-live-price-v2", (req, res) => {
  ...
  const fallbackCity = clean(req.query.destination_code || req.query.city);
  const fallbackAmount = money(req.query.amount, 0);

  let selected = null;

  for (let i = 0; i < rawHotels.length; i++) {
    const h = normalizeHotel(rawHotels[i], i, fallbackCity);
    const sameId = selectedId && String(h.hotel_id) === String(selectedId);
    const sameName = selectedName && norm(h.hotel_name) === selectedName;
  ...
  const currency = clean(
    selected?.currency ||
    selected?.first_rate?.display_currency ||
    req.query.currency ||
    CITY_CURRENCY[norm(fallbackCity)] ||
    "GBP"
  ).toUpperCase();

  const amount = money(
    selected?.price ||
    selected?.first_rate?.display_amount ||
    fallbackAmount,
    0
  );

  const rateKey = clean(
    selected?.first_rate?.rate_key ||
    selected?.hotel_id ||
    selectedId ||
    "selected-rate"
  );
  ...
  res.json({
    ok: true,
    live_payment_ready: amount > 0 && Boolean(rateKey),
    price_status: amount > 0
      ? "Live rate updated for the selected hotel. Final supplier confirmation still applies before payment."
      : "Selected hotel found, but live supplier rate must be confirmed before payment.",
    amount,
    currency,
