...
def booking_total_for_rooms(unit_amount, rooms):
    try:
...
def fetch_supplier_live_price_for_selected_hotel(supplier_hotel_id, checkin, checkout, guests, rooms, currency_hint=""):
    ...
    if r.status_code != 200:
        print("Supplier detail status:", r.status_code, str(data)[:220])
        return None
    ...
    if str(item.get("cant_book", "0")) not in ["0", "False", "false", ""]:
        return None
    ...
    return {
        ...
        "currency": clean(currency or currency_hint or "USD").upper(),
        "raw": item,
    }
...
def api_selected_hotel_live_check(
    ...
    unit_amount = "{:.2f}".format(float(supplier_live["unit_amount"]))
    total_amount = "{:.2f}".format(booking_total_for_rooms(unit_amount, rooms))
    currency = clean(supplier_live["currency"] or catalog.get("currency") or "USD").upper()

    # Basic sanity protection: block impossible prices from becoming payable.
    min_price = {"NGN": 15000, "KES": 1500, "ZAR": 600, "USD": 25, "GBP": 20, "EUR": 20, "AED": 120}
    if float(total_amount) < float(min_price.get(currency, 5)):
        return {
            "ok": True,
            "live_payment_ready": False,
            "payment_ready": False,
            "price_status": "Supplier price is being verified before payment.",
            "message": "Supplier returned an abnormal price and it requires verification before payment.",
            "amount": total_amount,
            "unit_room_amount": unit_amount,
            "rooms": int(rooms),
            "currency": currency,
        }
    ...
    """, (
        ...
        clean(destination_code).upper() or clean(catalog.get("city")).upper(),
        ...
        safe_json({
            "source": "rapidapi_properties_detail",
            "supplier_hotel_id": catalog.get("supplier_hotel_id"),
            "unit_room_amount": unit_amount,
            "rooms": int(rooms),
