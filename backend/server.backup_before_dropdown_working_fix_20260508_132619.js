...
if (STRIPE_SECRET_KEY) {
  try {
    ...
    console.log("Stripe package not available or failed to load:", error.message);
  }
}
...
function loadHotels() {
  ...
  for (const file of discoverInventoryFiles()) {
    const hotels = extractHotels(readJson(file));
  ...
  HOTEL_CACHE = [];
  HOTEL_SOURCE = "";
  console.log("No large real hotel catalog found.");
  return HOTEL_CACHE;
}
...
function valueFrom(raw, keys) {
  for (const key of keys) {
    ...
    let current = raw;
    for (const p of parts) current = current && current[p];
...
function normalizeHotel(raw, index, fallbackCity = "") {
  ...
  const city = valueFrom(raw, [
    "city", "cityName", "destination", "destination_name", "destinationName",
    "location.city", "address.city", "zoneName"
  ]) || fallbackCity;

  const country = valueFrom(raw, [
    "country", "countryName", "country_name", "location.country", "address.country",
  ...
  const id = valueFrom(raw, ["hotel_id", "id", "code", "hotelCode", "hotel_code", "property_id", "propertyId"]) || `hotel-${index + 1}`;
  const name = valueFrom(raw, ["hotel_name", "name", "hotelName", "property_name", "propertyName", "title"]) || `Hotel ${index + 1}`;
  ...
  return {
    ...
    city,
    country,
    area: valueFrom(raw, ["area", "neighbourhood", "neighborhood", "zone", "zoneName", "district"]),
    address: valueFrom(raw, ["address", "address_line", "addressLine", "full_address", "location.address"]),
    rating: valueFrom(raw, ["rating", "stars", "category", "categoryName"]) || "Available",
    image_url: clean(raw.image_url || raw.image || raw.main_image || raw.photo || raw.photos?.[0] || raw.images?.[0] || ""),
    currency,
    price,
    live_payment_ready: price > 0,
    price_status: price > 0 ? "Live or saved supplier rate available. Click to refresh selected rate." : "Supplier rate must be confirmed before payment.",
    first_rate: {
...
function buildDestinations(hotels) {
  ...
  const countries = [...map.entries()].map(([country, cityMap]) => ({
    country,
...
app.get("/", (req, res) => {
  ...
  res.json({ ok: true, service: "MySpace Hotel backend", port: PORT, real_hotels_loaded: hotels.length, hotel_source: HOTEL_SOURCE, stripe_enabled: Boolean(stripe) });
});

app.get("/health", (req, res) => {
  const hotels = loadHotels();
  res.json({ ok: true, real_hotels_loaded: hotels.length, hotel_source: HOTEL_SOURCE, stripe_enabled: Boolean(stripe) });
});
...
app.get("/api/real-catalog/destinations", (req, res) => {
  ...
  res.json({
    ...
    total_cities: countries.reduce((sum, x) => sum + x.cities.length, 0),
...
app.get("/api/real-catalog/search", (req, res) => {
  ...
  for (let i = 0; i < rawHotels.length; i++) {
    ...
    if (matchesHotel(hotel, country, city, area, keyword)) {
      hotels.push(hotel);
...
app.get("/api/hotels/selected-live-price-v2", (req, res) => {
  ...
  for (let i = 0; i < rawHotels.length; i++) {
    const h = normalizeHotel(rawHotels[i], i, city);
  ...
  const amount = money(selected?.price || selected?.first_rate?.display_amount || req.query.amount, 0);
  ...
  res.json({
    ...
    price_status: amount > 0 ? "Live rate updated for selected hotel. Continue securely when ready." : "Live supplier rate must be confirmed before payment.",
    amount,
    currency,
...
app.post("/reservation-request", async (req, res) => {
  try {
    ...
    return res.json({
      ...
      message: `Reservation request received. Reference: ${reservation_code}. Stripe is not enabled in this terminal environment.`
    });
  } catch (error) {
...
app.listen(PORT, "0.0.0.0", () => {
  ...
  console.log(`Cities: ${countries.reduce((sum, x) => sum + x.cities.length, 0)}`);
