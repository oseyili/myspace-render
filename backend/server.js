...
const DESTINATION_FILE_BACKEND = path.join(
  BACKEND_DATA,
  "live-destinations.json"
);

const DESTINATION_FILE_PUBLIC = path.join(
  PUBLIC,
  "live-destinations.json"
);

const HOTEL_GZ_FILE = path.join(
  BACKEND_DATA,
  "live-hotels.ndjson.gz"
);

const HOTEL_META_FILE = path.join(
  BACKEND_DATA,
...
function aliasesForCity(city) {
  ...
  if (CITY_ALIASES[key]) {
    for (const alias of CITY_ALIASES[key]) {
      set.add(alias);
    }
  }
...
async function searchCompressedHotels(
  country,
  city,
  limit
) {
  ...
  const wantedCityAliases =
    aliasesForCity(city);

  if (!fs.existsSync(HOTEL_GZ_FILE)) {
    return results;
  }
...
app.get("/status", (req, res) => {
  ...
  res.json({
    ...
    storage: fs.existsSync(HOTEL_GZ_FILE)
