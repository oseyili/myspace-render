...
function expectedPrice(ratePlan) {
  return (
    ...
    rooms: [
      {
        ...
        expectedPrice: expectedPrice(ratePlan)
      }
    ],
    leadGuest: {
...
async function run() {
  ...
  console.log("EXPECTED PRICE:", expectedPrice(ratePlan));
  ...
  fs.writeFileSync(
    path.join(LOG_DIR, "pre-book-test-1-dates-pax-fixed.json"),
    JSON.stringify(
      {
        step: "pre-book-test-1-dates-pax-fixed",
        url,
        status: res.status,
        ok: res.ok,
        request: body,
