...
const moduleFile = path.join(ROOT, "real-only-live-index.js");
...
if (!server.includes("attachRealOnlyRoutes")) {
  const backup = path.join(ROOT, "server_BACKUP_BEFORE_REAL_ONLY_ROUTES_" + Date.now() + ".js");
  fs.copyFileSync(serverFile, backup);

  const requireLine = 'const { attachRealOnlyRoutes } = require("./real-only-live-index");\n';
  server = requireLine + server;

  const appRegex = /(const|let|var)\s+app\s*=\s*express\s*\(\s*\)\s*;?/;
  const match = server.match(appRegex);

  if (!match) {
    throw new Error("Could not find Express app creation line in server.js");
  }

  server = server.replace(match[0], match[0] + "\nattachRealOnlyRoutes(app);");

  fs.writeFileSync(serverFile, server, "utf8");
  console.log("Patched server.js with real-only live routes.");
  console.log("Backup:", backup);
} else {
  console.log("server.js already has real-only live routes.");
}

console.log("Installed real-only live index.");
